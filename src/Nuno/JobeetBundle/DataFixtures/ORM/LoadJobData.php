<?php
namespace Nuno\JobeetBundle\DataFixtures\ORM;
 
use Doctrine\Common\Persistence\ObjectManager;
use Doctrine\Common\DataFixtures\AbstractFixture;
use Doctrine\Common\DataFixtures\OrderedFixtureInterface;
use Nuno\JobeetBundle\Entity\Job;
...
class LoadJobData extends AbstractFixture implements OrderedFixtureInterface
{
	public function load(ObjectManager $em)
	{
		...
		// Dynamic fixtures
		$jobCategories = array("category-administrator", "category-design", "category-design", "category-design", "category-manager", "category-programming", "category-programming", "category-programming");
		$jobtype = array("full-time", "full-time", "full-time", "full-time", "part-time");
		$jobLocation = array("Lisbon, Portugal", "Madrid, Spain", "Paris, France", "London, UK", "Rome, Italy", "Berlin, Germany", "Oslo, Norway", "Amsterdam, Netherlands", "Bern, Switzerland", "Brussels, Belgium", "Stockholm, Sweden, Copenhagen, Denmark");

		for($i = 0; $i <= 100; $i++)
		{
			$currCategory = array_rand($jobCategories, 1);
			switch ($jobCategories[$currCategory]) {
				case "category-administrator":
					$currJobPosition = "Administrator";
					break;
				case "category-design":
					$currJobPosition = "Web Designer";
					break;
				case "category-manager":
					$currJobPosition = "Project Manager";
					break;
				case "category-programming":
					$currJobPosition = "Web Developer";
					break;
			}

			$currJobType = array_rand($jobtype, 1);
			$currJobLocation = array_rand($jobLocation, 1);

			$job = new Job();
			$job->setCategory($em->merge($this->getReference($jobCategories[$currCategory])));
			$job->setType($jobtype[$currJobType]);
			$job->setCompany('Company '.$i);
			$job->setPosition($currJobPosition);
			$job->setLocation($jobLocation[$currJobLocation]);
